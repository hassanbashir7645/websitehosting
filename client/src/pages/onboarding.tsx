...
import { z } from 'zod';
import { UserPlus, CheckCircle, Clock, Plus, Edit, Trash2, FileCheck } from 'lucide-react';
...
export default function Onboarding() {
  ...
  const onboardingEmployees = employees?.filter((emp: Employee) => 
    emp.onboardingStatus !== 'completed'
  ) || [];

  const selectedEmployee = employees?.find((emp: Employee) => emp.id === selectedEmployeeId);

  const completedItems = checklists?.filter((item: OnboardingChecklist) => item.isCompleted).length || 0;
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Manage new employee onboarding process and share links</p>
        </div>
        ...
        {canManageOnboarding && selectedEmployeeId && (
          <div className="flex gap-2">
            <Button 
              onClick={async () => {
                try {
                  await apiRequest(`/api/onboarding/create-template/${selectedEmployeeId}`, {
                    method: 'POST'
                  });
            ...
            <CardContent className="p-0">
              {employeesLoading ? (
                ...
                <div className="divide-y divide-gray-200">
                  {onboardingEmployees.map((employee: Employee) => (
                    <div
                      key={employee.id}
          ...
          {selectedEmployee ? (
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-gray-600">{selectedEmployee.user.position}</p>
                  </div>
                  <div className="text-right">
            ...
            <Card className="stats-card">
              <CardContent className="p-12 text-center">
                ...
                <p className="text-gray-600">Choose an employee from the list to view their onboarding checklist</p>
              </CardContent>
            </Card>
          )}
        </div>
